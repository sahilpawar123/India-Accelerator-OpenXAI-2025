...
export default function Settings({ currentGoal, currentWakeTime, onSave, onClose, onHardReset, notificationPermission, onRequestPermission }: SettingsProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-sm text-white flex flex-col">
        ...
        <div className="mb-6">
          <h3 className="font-bold text-slate-300 mb-2">Notifications</h3>
          {notificationPermission === 'granted' && (
            <p className="text-sm text-green-400">Desktop notifications are enabled.</p>
          )}
          {notificationPermission === 'default' && (
            <button onClick={onRequestPermission} className="w-full bg-blue-600 text-white font-semibold p-2 rounded-lg hover:bg-blue-700 transition">
              Enable Notifications
            </button>
          )}
          {notificationPermission === 'denied' && (
            <p className="text-sm text-red-400">You have blocked notifications. Please enable them in your browser's site settings.</p>
          )}
        </div>
        ...
        <div className="border-t border-slate-700 mt-6 pt-6">
           <button onClick={onHardReset} className="w-full bg-red-600/80 text-white font-semibold p-2 rounded-lg hover:bg-red-700 transition">
             Reset All Data
           </button>
